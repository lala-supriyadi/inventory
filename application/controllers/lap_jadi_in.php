...
class lap_jadi_in extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->load->model('lap_jadi_in_model');
    }
    ...
    public function index(){
        ...
        $data['fungsi'] = "lap_jadi_in/tanggal_jadi_in";
        $data['title'] = "DVStar";
        $content = "laporan/jadi_in/tanggal";
        $data['judule'] = "Laporan";

        $this->template->output($data,$content);

    }
    function tanggal_jadi_in(){
        ...
        $data['title']="DVStar";
        $data['fungsi'] = "lap_jadi_in/tanggal_jadi_in";
        $data['judule']="Laporan Barang Masuk";
        $tgl2=$this->input->post('tanggal1');
        $tgl3=$this->input->post('tanggal2');
        $content = "laporan/jadi_in/cari_tanggal";
        $data['tgl_mulai']= $tgl2;
        $data['tgl_selesai']= $tgl3;
        $data['lap'] = $this->lap_jadi_in_model->tanggal($tgl2,$tgl3)->result();
        $this->template->output($data, $content);
    }
    ...
    public function pdf($from, $to) {
        ...
        $pdf->Cell(0, 8, "Laporan Barang Jadi Masuk " . $from . " - " .$to, 0, 1, 'L');
    ...
    public function print_laporan($tgl_mulai,$tgl_selesai){
        ...
        $pdf->Cell(0, 8, "Laporan Barang Masuk ", 0, 1, 'C');
        $pdf->SetFont('Arial', 'B', 10);
        ...
        $pdf->Cell(35, 7, 'Model Barang', 1, 0, 'C',1);
        $pdf->Cell(25, 7, 'Jenis', 1, 0, 'C',1);
        $pdf->Cell(40, 7, 'Motif Barang', 1, 0, 'C',1);
        $pdf->Cell(50,7, 'Tanggal Masuk', 1, 0, 'C',1);
        $pdf->Cell(30,7, 'jumlah', 1, 0, 'C',1);
        $pdf->Ln();
       
        $simpan = $this->lap_jadi_in_model->tanggal($tgl_mulai,$tgl_selesai)->result();
        ...
        foreach ($simpan as $p) {
            ...
            $pdf->Cell(35, 7, $p->model_barang, 1, 0, 'C',1);
            $pdf->Cell(25, 7, $p->jenis_barang, 1, 0, 'C',1);
            $pdf->Cell(40, 7, $p->motif_barang, 1, 0, 'C',1);
            $pdf->Cell(50, 7, $p->tgl_masuk, 1, 0, 'C',1);
